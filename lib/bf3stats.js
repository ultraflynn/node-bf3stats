var request = require("./bf3stats-request");
var cache = require("./bf3stats-cache");
...
BF3Stats.prototype.onlinestats = function(callback) {
	request.call(null, "global", "onlinestats", handle(callback));
};
...
// TODO Prototype for when the request issues events rather than calling back

// signed request
BF3Stats.prototype.getkey = function(time, clientident, callback) {
	var execution = request.create();
	attachListeners(executions);
	execution.call({
		time: time,
		ident: this.ident,
		clientident: clientident
	}, this.platform, "getkey");
};

function attachListeners(execution) {
	execution.on("request timeout (30 secs) exceeded", function() {
	});
	execution.on("http error", function() {
	});
	execution.on("api error", function() {
	});
	execution.on("data error", function() {
	});
}
